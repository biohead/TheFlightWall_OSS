...
2) Use local data from an ADSB reciever (running tar1090)
...
3. Copy the generated key and add it to [APIConfiguration.h](firmware/config/APIConfiguration.h)
...
On first boot, the ESP32 will broadcast an AP and you can configure it from there.
Alternatively, set your location to track flights by updating the following values in [UserConfiguration.h](firmware/config/UserConfiguration.h):
...
2. `FlightDataFetcher` enriches each callsign: AeroAPI for route/airline/aircraft, FlightWall CDN for human-readable names, `LocalLogoStore` for the airline logo bitmap.  Results are cached in RAM (default 30 min) to minimise API calls.
...
3. **Build and upload**:
   ...
   - Click "Build Filesystem Image" in Platform submenu
